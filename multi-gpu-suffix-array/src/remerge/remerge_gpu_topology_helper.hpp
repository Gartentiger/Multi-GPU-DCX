            ...
            for (uint node = 0; node < NUM_GPUS; ++node) {
                ...
                for (const InterNodeCopy& c : copies[node]) {
                    ...
                    if (c.src_node == world_rank()) {
                        ...
                        std::span<uint64_t> sb(reinterpret_cast<uint64_t>(src_k_buff) + c.src_index, c.len);
                    ...
                    if (c.dest_node == world_rank()) {
                        ...
                        std::span<uint64_t> rb(reinterpret_cast<uint64_t>(dest_k_buff) + c.dest_index, c.len);
                        comm_world().recv(recv_buf(rb), recv_count(c.len));
                    }
                    if (do_values) {
                        if (c.src_node == world_rank()) {
                            const value_t* src_v_buff = mnodes[c.src_node].info.values;
                            std::span<uint32_t> sb(reinterpret_cast<uint32_t>(src_v_buff) + c.src_index, c.len);
                            comm_world().send(send_buf(sb), send_count(c.len), destination((size_t)c.dest_node));
                        }
                        if (c.dest_node == world_rank()) {
                            value_t* dest_v_buff = mnodes[c.dest_node].info.value_buffer;
                            std::span<uint32_t> rb(reinterpret_cast<uint32_t>(dest_v_buff) + c.dest_index, c.len);
                            comm_world().recv(recv_buf(rb), recv_count(c.len));
                        }
