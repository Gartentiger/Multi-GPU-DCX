...
private:
    ...
    void initial_sort_dcx()
    {
...

        {
            ...
            cub::DoubleBuffer<kmerDCX> keys(gpu.Kmer, gpu.Kmer_buffer);
            cub::DoubleBuffer<sa_index_t> values(gpu.Isa, gpu.Sa_index);
            size_t temp_storage_bytes = 0;
            ...
            ASSERT(keys.Current() == gpu.Kmer_buffer ? values.Current() == gpu.Sa_index : values.Current() == gpu.Isa);
        }
        ...
        for (uint gpu_index = 0; gpu_index < NUM_GPUS; ++gpu_index)
        {
            ...
            merge_nodes_info[gpu_index] = { gpu.working_len, 0, gpu_index,
                ...
                in_buffer[gpu_index] ? gpu.Kmer : gpu.Kmer_buffer,
                in_buffer[gpu_index] ? gpu.Isa : gpu.Sa_index ,
                nullptr, nullptr };
    ...
    void write_initial_ranks()
    {
        ...
        sa_index_t* other_buffer = in_buffer[gpu_index] ? gpu.Isa : gpu.Sa_index;
        ...
        //printf("last element\n");
        kernels::write_ranks_diff_multi _KLC_SIMPLE_(gpu.working_len, mcontext.get_gpu_default_stream(gpu_index))(current_buffer, last_element_prev, gpu.offset + 1, 0, other_buffer, gpu.working_len);
        CUERR;
        //}
        ...
        printArrayss << <1, 1 >> > (current_buffer, other_buffer, std::min(80UL, gpu.working_len), world_rank());
        mcontext.sync_all_streams();
        comm_world().barrier();
        ...
        for (uint gpu_index = 0; gpu_index < NUM_GPUS; ++gpu_index)
        {
            ...
            if (gpu.working_len > 0)
            {
                if (gpu_index == world_rank())
                {
                    ...
                    if (initial) {
                        input_buffer = in_buffer[gpu_index] ? gpu.Isa : gpu.Sa_index;
